...
export default class OffcanvasMenuPlugin extends Plugin {
  ...
  _getLinkEventHandler(event, link) {
    if (!link) {
      ...
      const initialContentElement = DomAccess.querySelector(
        document,
        this.options.initialContentSelector
      );
    ...
    const url =
      DomAccess.getAttribute(link, "data-href", false) ||
      DomAccess.getAttribute(link, "href", false);
    ...
    if (
      link.classList.contains(this.options.homeBtnClass) ||
      link.classList.contains(this.options.backBtnClass)
    ) {
  ...
  _createOverlayElements() {
    ...
    if (offcanvasMenu) {
      ...
      this._overlay = OffcanvasMenuPlugin._createNavigationOverlay(
        offcanvasMenu
      );
    }
  ...
  static _createPlaceholder(container) {
    const offcanvas = OffcanvasMenuPlugin._getOffcanvas();
    const currentPlaceholder = offcanvas.querySelector(
