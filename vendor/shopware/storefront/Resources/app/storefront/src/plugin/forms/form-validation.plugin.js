...
export default class FormValidation extends Plugin {
  ...
  _registerEvents() {
    ...
    this._registerValidationListener(
      this.options.equalAttr,
      this._onValidateEqualTrigger.bind(this),
      ["change"]
    );
    this._registerValidationListener(
      this.options.equalAttr,
      Debouncer.debounce(
        this._onValidateEqual.bind(this),
    ...
    // length validation
    this._registerValidationListener(
      this.options.lengthAttr,
      this._onValidateLength.bind(this),
      ["change"]
    );
  }
  ...
  _onValidateEqualTrigger(event) {
    const selector = DomAccess.getDataAttribute(
      event.target,
      this.options.equalAttr
    );
    const fields = DomAccess.querySelectorAll(
      this.el,
      `[${this.options.equalAttr}='${selector}']`
    );
    ...
    if (confirmFieldValue.length > 0) {
      Iterator.iterate(fields, (field) => {
        field.dispatchEvent(
          new CustomEvent(this.options.eventName, { target: event.target })
        );
      });
    }
  ...
  _onValidateLength(event) {
    ...
    const expectedLength = DomAccess.getDataAttribute(
      event.target,
      this.options.lengthAttr
    );
  ...
  _showInvalidMessage(field, attribute) {
    ...
    const message = DomAccess.getDataAttribute(
      field,
      `${attribute}-message`,
      false
    );
    ...
    if (message) {
      if (!parent.querySelector(".js-validation-message")) {
        field.insertAdjacentHTML(
          "afterEnd",
          `<div class="invalid-feedback js-validation-message" data-type="${attribute}">${message}</div>`
        );
      }
  ...
  _hideInvalidMessage(field, attribute) {
    ...
    if (parent) {
      const message = DomAccess.querySelector(
        parent,
        `.js-validation-message[data-type=${attribute}]`,
        false
      );
      if (message) {
        message.remove();
