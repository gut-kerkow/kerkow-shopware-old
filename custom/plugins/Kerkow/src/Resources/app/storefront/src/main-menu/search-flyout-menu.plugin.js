 ...
 */
export default class SearchFlyoutMenuPlugin extends Plugin {
  static options = {
    /**
     ...
     */
    closeSelector: ".js-close-search-flyout-menu",

    /**
     * Id attribute for the flyout.
     * Should be the same as 'triggerDataAttribute'
     */
    flyoutIdDataAttribute: "data-search-flyout-menu",

    /**
     * Trigger attribute for the opening elements.
     * Should be the same as 'flyoutIdDataAttribute'
     */
    triggerDataAttribute: "data-search-flyout-menu-trigger",

    //*Menu entry to add underline
    searchWidgetMainNavigationLink: ".main-navigation-search",
  };
  ...
  _registerEvents() {
    ...
    Iterator.iterate(this._triggerEls, (el) => {
      el.addEventListener(openEvent, this._openSearchFlyout.bind(this, el));
  ...
  _openFlyout(flyoutEl, triggerEl) {
    if (!this._isOpen(triggerEl)) {
      ...
      this.el
        .querySelector(this.options.searchWidgetMainNavigationLink)
  ...
  _openSearchFlyout(triggerEl, event) {
    ...
    if (!this._isOpen(triggerEl)) {
      SearchFlyoutMenuPlugin._stopEvent(event);
    }
