...
export default class PayonePaymentCreditCard extends Plugin {
    ...
    this._createScript(() => {
      ...
      if (field) {
        field.addEventListener(
          "change",
          this._handleChangeSavedCard.bind(this)
        );
      }
  ...
  _cardDetectionCallback(detectedCardtype) {
    ...
    const src =
      "https://cdn.pay1.de/cc/" +
      detectedCardtype.toLowerCase() +
      "/xl/default.png";
  ...
  _handleOrderSubmit(event) {
    ...
    if (this.orderFormDisabled) {
      window.payoneCreditCardCheckCallback = this._payoneCheckCallback.bind(
        this
      );
  ...
  _handleChangeSavedCard() {
    ...
    if (savedCardPan.options[savedCardPan.selectedIndex].value) {
      [...document.getElementsByClassName("credit-card-input")].forEach(
        function (element) {
