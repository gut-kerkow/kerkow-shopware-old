...
use Kerkow\ProductBundles\Core\Content\ProductBundles\ProductBundlesCollection;
...
{
    public const TYPE = 'kerkowproductbundles';
    public const DISCOUNT_TYPE = 'kerkowproductbundles-discount';
    public const DATA_KEY = 'Kerkow_product_bundles-';
     ...
     */
    private function fetchBundles(LineItemCollection $bundleLineItems, CartDataCollection $data, SalesChannelContext $context): ProductBundlesCollection
    {
        ...
        /** @var ProductBundlesCollection $bundles */
        $bundles = $this->bundleRepository->search($criteria, $context->getContext())->getEntities();

        return $bundles;
    }

    private function enrichBundle(LineItem $bundleLineItem, ProductBundlesEntity $bundle): void
    {
    ...
    private function addMissingProducts(LineItem $bundleLineItem, ProductBundlesEntity $bundle): void
    {
        $bundleProducts = $bundle->getProducts();
        if ($bundleProducts === null) {
            throw new \RuntimeException(sprintf('Bundle %s has no products', $bundle->getName()));
    ...
    private function addDiscount(LineItem $bundleLineItem, ProductBundlesEntity $bundle, SalesChannelContext $context): void
    {
    ...
    private function createDiscount(ProductBundlesEntity $bundleData, SalesChannelContext $context): ?LineItem
    {
